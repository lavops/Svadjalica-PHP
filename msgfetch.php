...
if($room>0){$query='SELECT * FROM '.$dbss['prfx']."_messages WHERE touserid=0 AND roomid=$room AND timestamp<$tpoi ORDER BY id DESC LIMIT 30 OFFSET 0";}
if($tuid>0){$query='SELECT * FROM '.$dbss['prfx']."_messages WHERE ((touserid=$tuid AND userid=$userid) OR (touserid=$userid AND userid=$tuid)) AND timestamp<$tpoi ORDER BY id DESC LIMIT 30 OFFSET 0";}
