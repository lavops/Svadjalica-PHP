...
// exit
if($_POST['msg']=='1'){
    $res=neutral_query('SELECT * FROM '.$dbss['prfx'].'_users WHERE id='.$userid);
    ...
    $touser=0;
    $tousername="";
    $line=neutral_escape(''.$name." je upravo napustio chat!",2048,'str');
    $color=10;
    $attach=0;
    neutral_query('INSERT INTO '.$dbss['prfx']."_messages VALUES(NULL,$roomid,232,$usergroup,'Svadjalica',$touser,'$tousername','$line',$color,$attach,$timestamp)");
    neutral_query('DELETE FROM '.$dbss['prfx'].'_online WHERE id='.$userid);die('{}');
}
